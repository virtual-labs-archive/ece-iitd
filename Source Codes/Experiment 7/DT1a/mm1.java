...
public class mm1 extends Applet implements ActionListener, Runnable {
...
public static double uniform() {
    ...
    public static double GenerateArrival(double l) {
        double time = (int) Math.ceil(Math.log(uniform()) / Math.log(1.0 - l));
        return time;
    }

    public static double GenerateDeparture(double l) {
        double time =  time = (int) Math.ceil(Math.log(uniform()) / Math.log(1.0 - l));
        return time;
    }
    ...
    public void Join(Graphics g, Point p1) {
        
        g.setColor(Color.RED);
        g.fillOval((int) ((p1.x - 50) * scale + 47), (int) (270 - (270 - p1.y) * yscale-5),5,5) ;
       g.setColor(PathColor);
       g.drawLine((int) ((p1.x - 50) * scale + 50), (int) (270 - (270 - p1.y) * yscale),(int) ((p1.x - 50) * scale + 50),270);
    }
   ...
   public void HandleError(int e) {
        ...
        if (e == 11) {
            g.drawString("*Error In Input In p (Arrival)", a, b + d * x);
            x++;
        }
        if (e == 12) {
            g.drawString("*p Should Be between 0 and 1", a, b + d * x);
            x++;
        }
        if (e == 81) {
        ...
        if (e == 21) {
            g.drawString("*Error in Input In q (departure)", a, b + d * x);
            x++;
        }
        if (e == 22) {
            g.drawString("*q  Should Be between 0 and 1", a, b + d * x);
            x++;
        }
        if (e == 31) {
    ...
    public void init() {
        ...
        label1= new Label();
        label1.setText("Arrival Probability(p) : ");
        label2= new Label();
        label2.setText("Departure Probability (q) : ");
        label3= new Label();
      ...
      //  CalculateSteadyStatemeasures();
        ...
        while (true) {
           ...
           // points.addLast(new Point(nextX, nextY));
            ...
            System.out.println("System Size : " + SystemSize + " ");
